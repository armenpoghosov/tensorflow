...
struct NodeOutHash
{
    ...
    {
        return Hash64(reinterpret_cast<const char*>(&x.node), sizeof(Node*), x.index);
    }
};
...
void SymbolicGradientBuilder::BackpropAlongEdge(const NodeOut& dst_grad, const NodeOut& src)
{
...

    {
        ...
        for (int i = 0; i < num_y; ++i)
        {
            ...
            for (const Edge* e : y->in_edges())
            {
                ...
                BackpropAlongEdge(y_grad_node_outputs_[i], NodeOut { e->src(), e->src_output() });
            }
        }
    }
...
Status AddSymbolicGradients(gtl::ArraySlice<NodeOut> y_node_outputs,
    gtl::ArraySlice<NodeOut> x_node_outputs, gtl::ArraySlice<NodeOut> y_grad_node_outputs,
    std::vector<NodeOut>* x_grad_node_outputs, Graph* graph)
{
    SymbolicGradientBuilder builder(y_node_outputs, x_node_outputs,
        y_grad_node_outputs, x_grad_node_outputs, graph);
