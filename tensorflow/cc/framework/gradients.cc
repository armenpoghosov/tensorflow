...
struct OutputHash
{
    uint64 operator () (Output const& x) const
    {
        return x.hash();
    }
};
...
struct OutputEq
{
    bool operator () (Output const& x, Output const& y) const
    {
...
public:
    ...
    SymbolicGradientBuilder(Scope const& scope, ops::GradOpRegistry const* registry,
        std::vector<Output> const& outputs, std::vector<Output> const& inputs,
...
private:
    ...
    Status ProcessWhileLoop(Node* exit_node, Output const& summed_grads);
    ...
    std::unordered_set<int> GetStopBackpropNodes(
        std::vector<bool> const& reachable_nodes,
        std::unordered_set<int> const& output_nodes);

    Scope const&                scope_;
    ops::GradOpRegistry const*  registry_;
    std::vector<Output> const&  outputs_;
    std::vector<Output> const&  inputs_;
    std::vector<Output> const&  grad_inputs_;
...
SymbolicGradientBuilder::SymbolicGradientBuilder(
        Scope const& scope, ops::GradOpRegistry const* registry,
        std::vector<Output> const& outputs, std::vector<Output> const& inputs,
        std::vector<Output> const& grad_inputs, std::vector<Output>* grad_outputs)
    :
    scope_(scope),
...
Status SymbolicGradientBuilder::BackpropAlongEdge(Output const& dst_grad, Output const& src)
{
    if (src.node() == nullptr)
    {
    ...
    std::unordered_set<int> const& output_nodes)
{
    ...
    while (!queue.empty())
    {
        ...
        for (const Edge* e : n->out_edges())
        {
            ...
            if (e->IsControlEdge() || !reachable_nodes[e->dst()->id()])
                continue;
...
Status SymbolicGradientBuilder::Initialize()
{
    ...
    {
        return errors::InvalidArgument("Must specify a gradient input for each output.");
    }

    std::vector<bool> reachable_nodes = GetReachableNodes();
    for (Output const& input : inputs_)
    {
        if (!reachable_nodes[input.node()->id()])
        {
            return errors::InvalidArgument("Cannot compute the partial derivative for node '",
                input.node()->name(), "' as it's unreachable from the output node(s).");
        }
    }
    ...
    pending_.resize(scope_.graph()->num_node_ids(), 0);
    {
        ...
        while (!queue.empty())
        {
            ...
            if (output_nodes.find(n->id()) != output_nodes.end())
            {
                ...
                for (Output const& output : outputs_)
                {
                    if (output.node() == n)
                    {
                        ++num_expected_backprops;
...
Status SymbolicGradientBuilder::SumGradients(Output const& src, Output* grad)
{
    auto iter = backprops_.find(src);
    if (iter == backprops_.end())
    {
        return errors::Internal("Unable to find backprop list for node.id ", src.node()->name());
    }
...
bool SymbolicGradientBuilder::IsPrimitiveOpWithNoGrad(string const& opname)
{
...
Status SymbolicGradientBuilder::CallGradFunction(
    Operation const& op, std::vector<Output> const& grad_inputs,
...
Status SymbolicGradientBuilder::ProcessWhileLoop(Node* exit_node, Output const& summed_grads)
{
    ...
    {
        return errors::Unimplemented("Missing gradient into while loop not yet implemented");
    }
    ...
    Scope while_scope = scope_.NewSubScope(strings::StrCat(while_ctx->frame_name(), "_grad"));
    std::vector<Output> dy;
    for (Node* n : while_ctx->exit_nodes())
        dy.push_back(backprops[n]);
    ...
    while (!ready_.empty())
    {
        ...
        if (IsPrimitiveOpWithNoGrad(n->type_string()) || num_no_grad == num_y)
        {
            ...
            for (const Edge* e : n->in_edges())
            {
                ...
                TF_RETURN_IF_ERROR(BackpropAlongEdge(NoGradient(), {e->src(), e->src_output()}));
            }
...
Status AddSymbolicGradients(Scope const& scope,
    std::vector<Output> const& outputs, std::vector<Output> const& inputs,
    std::vector<Output> const& grad_inputs, std::vector<Output>* grad_outputs)
{
    SymbolicGradientBuilder builder(scope, ops::GradOpRegistry::Global(),
        outputs, inputs, grad_inputs, grad_outputs);
    return builder.AddGradients();
}

Status AddSymbolicGradients(Scope const& scope, std::vector<Output> const& outputs,
    std::vector<Output> const& inputs, std::vector<Output>* grad_outputs)
{
    std::vector<Output> grad_inputs;
    grad_inputs.reserve(outputs.size());

    for (Output const& output : outputs)
    {
        grad_inputs.emplace_back(ops::OnesLike(scope, output));
    }

    return AddSymbolicGradients(scope, outputs, inputs, grad_inputs, grad_outputs);
}
